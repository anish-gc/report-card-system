...
class ReportCardQuerySet(models.QuerySet):
    ...
    def with_marks(self):
        ...
        return self.prefetch_related(
            Prefetch(
                'marks',
                queryset=models.get_model('students', 'Mark').objects.select_related('subject')
            )
        )
...
class ReportCardManager(models.Manager):
    ...
    def calculate_year_averages(self, student_id: int, year: int) -> Dict[str, Any]:
        ...
        marks_qs = (
            Mark.objects
            .filter(report_card__student_id=student_id, report_card__year=year)
    ...
    def bulk_publish(self, report_card_ids: List[int]) -> int:
        """Bulk publish multiple report cards."""
        return self.filter(id__in=report_card_ids).update(is_published=True)
    
    def bulk_unpublish(self, report_card_ids: List[int]) -> int:
        """Bulk unpublish multiple report cards."""
        return self.filter(id__in=report_card_ids).update(is_published=False)
